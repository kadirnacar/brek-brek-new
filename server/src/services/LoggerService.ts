...
export class LoggerService {
  ...
  public error(error): winston.Logger {
    // this.setErrorStack();
    console.error(error);
    if (this.winstonLogger) return this.winstonLogger.error(error);
  }

  public info(info): winston.Logger {
    // this.setErrorStack();
    console.info(info);
    if (this.winstonLogger) return this.winstonLogger.info(info);
  }

  public static async init(options?: winston.LoggerOptions) {
    if (!this.logger) this.logger = new LoggerService(options);
  }
}
LoggerService.init();
